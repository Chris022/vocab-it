...
class DefaultSetRepository @Inject constructor(
    ...
    override suspend fun getAllWithFlashcards(type: SetType): List<SetAndFlashCards> {
        ...
        if(type == SetType.Reading || type == SetType.Writing || type == SetType.ReadWrite) out.addAll(setSource.allWithFlashcards(
            SetType.ReadWrite))
    ...
    override suspend fun getAll(type: SetType): List<Set> {
        ...
        //first add all ReadWrite
        if(type == SetType.Reading || type == SetType.Writing || type == SetType.ReadWrite) out.addAll(setSource.all(
            SetType.ReadWrite))
        //than add the Read or Write
